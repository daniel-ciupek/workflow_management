            ...
            <div class="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div x-data="{
                    ...
                    down(e) {
                        ...
                        else if (!this.p2 && e.pointerId !== this.p1.id) this.p2 = {id: e.pointerId, x: e.clientX, y: e.clientY};
                        if (this.p1 && this.p2) this.lastDist = Math.hypot(this.p2.x - this.p1.x, this.p2.y - this.p1.y);
                    },
                    onMove(e) {
                        if (!this.p1) return; this.moved = true;
                        if (this.p2) {
                            ...
                            if (this.lastDist > 0) { const f = dist / this.lastDist; this.scale = Math.max(1, Math.min(12, this.scale * f)); if (this.scale === 1) { this.tx = 0; this.ty = 0; } }
                            this.lastDist = dist;
                        } else if (e.pointerId === this.p1.id && this.scale > 1) {
                            this.tx += e.clientX - this.p1.x; this.ty += e.clientY - this.p1.y;
                    ...
                    @if(!empty($viewingTask->attachments))
                        ...
                        <div x-show="lightbox" x-transition @keydown.escape.window="close()" class="fixed inset-0 bg-black z-[200] flex flex-col" style="display:none;">
                            ...
                            <div class="flex-1 overflow-hidden flex items-center justify-center"
                                 @pointerdown="down($event)" @pointermove="onMove($event)" @pointerup="onEnd($event)" @pointercancel="onCancel()"
                                 @wheel.prevent="zoom($event)" style="touch-action: none; user-select: none;">
                                <img :src="lightbox" :style="`transform: translate(${tx}px,${ty}px) scale(${scale}); cursor: ${scale>1?'grab':'zoom-in'}; transform-origin:center;`"
                                     class="max-w-full max-h-full object-contain select-none" @dblclick.stop="scale===1?(scale=2):reset()" draggable="false" />
                            </div>
                            <div class="text-center text-white/30 text-xs py-2 shrink-0 select-none"
                                 x-text="('ontouchstart' in window) ? 'Pinch to zoom · Drag to pan · Tap background to close' : 'Scroll to zoom · Drag to pan · Double-click to zoom · ESC to close'"></div>
                        </div>
                    @else
